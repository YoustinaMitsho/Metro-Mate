...
std::vector<int> Station::CreateStationLineVec(std::string name, std::vector<int> vec_line){
    for (int i = 0; i < stations.size(); ++i) {
        qDebug() << name  << " test " << stations[i].name << "\n";
        if(name == stations[i].name)
        {
            vec_line.emplace_back(stations[i].lines);
            qDebug()<<"line"<<stations[i].lines;
        }
    }
    return vec_line;
}
...
{
    ifstream file ("D:/QT/MetroMate/img/stations_name.txt");
    if (!file.is_open()){
...
{
    ofstream outFile("D:/QT/MetroMate/img/stations_name.txt");
...
void Station::readstations(){
    QFile file("D:/QT/MetroMate/img/stations_name.txt");
