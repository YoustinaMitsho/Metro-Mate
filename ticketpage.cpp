...
void TicketPage::on_code_editingFinished()
{
    if(ui->code->text() == "123")
    {
       UserClass::thisuser.UserSub.no_of_trips=0;
        ticket_cost = UserClass::thisuser.Calc_cost(ticket.Start_station,ticket.End_station);
        int ticket_can_be_add= UserClass::thisuser.Add_Ticket(ticket_cost,ticket);
        ...
        if(ticket_can_be_add==1){
            ...
            for(auto& it1:UserClass::users){
                if(it1.Username==UserClass::thisuser.Username){
                    it1.Add_Ticket(ticket_cost,ticket);
                }
            }
            emit SwitchToVerf();
