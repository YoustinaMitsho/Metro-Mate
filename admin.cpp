    ...
    ui(new Ui::admin)
{
   ...
   ui->label_11->setPixmap(st.scaled(ui->label_11->width(), ui->label_11->height(),Qt::KeepAspectRatio));
    ...
    ui->textBrowser->setStyleSheet("QTextBrowser {"
                                   "font-size: 16pt;"      // Keep the font size as it is
                                   "font-weight: bold;"    // Keep the text bold
                                   "padding-top: 10px;"    // Add padding to the top
                                   "}");
    ui->textBrowser_2->setStyleSheet("QTextBrowser {"
                                   "font-size: 16pt;"      // Keep the font size as it is
                                   "font-weight: bold;"    // Keep the text bold
                                   "padding-top: 10px;"    // Add padding to the top
                                   "}");
    // Set the alignment of the text within the QTextBrowser
